...
const CostingForm = React.createClass({
  propTypes: {
  ...
  ingredientFields(ingredients, vendorOptions) {
    ...
    return ingredients.map(model => {
      ...
      var costPerGram; 
      if(model.current_amount == 0.0 || model.cost == 0.0 ){
        costPerGram = 0.0;
      }
      else {
        costPerGram = Math.round(model.current_amount / 453.5)
      }
    return (<div className={"row ingredient " + model.hidden} key={model.id}>
      <div className="small-12 columns">
        ...
        <div className="ingredient-current-amount-lb" key={`${model.id}-current_amount_lb`}>
          <label className="hide-for-large-up">Amount in LB</label>
