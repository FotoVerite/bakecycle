...
describe ShipmentService do
  ...
  context "creates shipments" do
    it "creates shipments for orders and dates where the production date is today" do
      create(:order, bakery: bakery, start_date: today, force_total_lead_days: 1, order_item_count: 1)
      create(:order, bakery: bakery, start_date: tomorrow, force_total_lead_days: 1, order_item_count: 1)
      create(:order, bakery: bakery, start_date: today + 2.days, force_total_lead_days: 1, order_item_count: 1)
    ...
    it "creates shipments for all clients of bakery" do
      create(:order, bakery: bakery, start_date: today, force_total_lead_days: 1, order_item_count: 1)
      create(:order, bakery: bakery, start_date: today + 1.day, force_total_lead_days: 1, order_item_count: 1)
      create(:order, bakery: bakery, start_date: today + 2.days, force_total_lead_days: 1, order_item_count: 1)
      shipment_service.run
      expect(Shipment.count).to eq(2)
    end
